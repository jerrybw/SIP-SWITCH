...
| **#64** | **网关-节点归属表 `gateway_node`（D11 多节点分片数据底座）** | **✅** | 已定义并落库（commit `ea5c155`）：`ensure_gateway_node_table` 幂等建表 + `01-schema.sql` + `session.py` 启动注册；dev 库已建表、gateway 重启无报错。`uk_gateway(gateway_id)` 保证注册型网关(auth_type=1)单选唯一归属节点；FK→`gateway.id`/`fs_node.node_uuid`(ON DELETE CASCADE)。点对点网关(auth_type=0)全量下发不在此表。**2026-09-10 剩余部分全部完成**：① `db/models.py` 新增 `GatewayNode` ORM；`migrate.py`+`session.py` 增 `ensure_gateway_node_backfill`（存量注册型网关回填到首个 `fs_node`，避免升级后从所有节点消失）。② `crud.py`：`_sync_gateway_node`（auth_type=1 upsert / auth_type=0 删行）+ `_validate_gateway_node`（**落库前**校验：注册型必选节点 + 同节点 `ip+port+账号` 三要素唯一，避免校验失败留下无归属孤儿网关）+ `_enrich_gateway_node`（列表/单条回带 `node_uuid`）。③ `fs_sofia_config._gateways_block` 按 `NODE_UUID` 过滤下发（注册型只下发给归属节点、点对点全量）。④ `route/service.py::select_outbound_gateway` 按 `NODE_UUID` 筛候选网关（failover/并发范围随之处在节点内，D11 设计预期）。⑤ `admin.js` 网关表单加「归属节点」`select-src`(`/api/nodes`)，仅注册模式显示且必填(`requiredIf`)，点对点模式隐藏并清空。**端到端验证**：创建归属 node2 的注册网关 → node1 下发不含、node2 含；改为点对点 → 两节点均含；同节点三要素重复 → 400 且不留孤儿；存量 `test-register-gw`(auth_type=1) 启动即回填 node1
